...
class Subject extends GeneralFunctions
{
    ...
    public function insert($subject_name, $subject_fees, $semester_id, $branch_id)
    {
        ...
        $query = "INSERT INTO $this->tableName (subject_name, subject_fees,semester_id,
branch_id, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?)";

        $current_date = date("Y-m-d h:i:sa");
        $created_by = $_SESSION['member_id'];
        $preparedStatement = $this->connection->prepare($query);
        $preparedStatement->bind_param("ssiiis", $subject_name, $subject_fees,
            $semester_id, $branch_id, $created_by, $current_date);
     ...
     public function update($id, $subject_name, $subject_fees)
    {
        ...
        $query = "UPDATE $this->tableName SET subject_name = ?, subject_fees = ?,
updated_at = ?, updated_by = ? WHERE id = ?";
        $current_date = date("Y-m-d h:i:sa");
        $updated_by = 1;

        $preparedStatement = $this->connection->prepare($query);
        $preparedStatement->bind_param("sssii", $subject_name, $subject_fees,
     ...
     public function displayRecordsWithPagination($records_per_page){
        ...
        while ($row = mysqli_fetch_assoc($result_set)) {
            ...
            <tr>
                ...
                <td><?php echo $row['subject_fees']; ?></td>
                <td>
