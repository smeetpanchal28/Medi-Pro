...
class Branch extends GeneralFunctions
{
    ...
    public function insert($branch_name, $branch_code)
    {
        ...
        $query = "INSERT INTO $this->tableName (branch_name, branch_code,
created_at, updated_at, updated_by, deleted) VALUES (?, ?, ?, ?, ?, ?)";
        ...
        $preparedStatement->bind_param("ssssii", $branch_name, $branch_code,
            $current_date, $current_date, $updated_by, $deleted);
    ...
    public function delete($id)
    {
        ...
        $deleted = 1;
        $deleted_by = 1;
        $query = "UPDATE $this->tableName SET deleted = $deleted,
deleted_at = '$current_date', deleted_by = '$deleted_by' WHERE id = $id";
     ...
     public function update($id, $branch_name, $branch_code)
    {
        ...
        $query = "UPDATE $this->tableName SET branch_name = ?, branch_code = ?,
updated_at = ?, updated_by = ? WHERE id = ?";
        $current_date = date("Y-m-d h:i:sa");
        $updated_by = 1;

        $preparedStatement = $this->connection->prepare($query);
        $preparedStatement->bind_param("sssii", $branch_name, $branch_code,
     ...
     public function displayRecordsWithPagination($records_per_page){
        ...
        if(isset($_POST['key'])){
            $key = $_POST['key'];
            $condition = "AND (branch_name like '%$key%' or
            branch_code like '%$key%') ";
        }
