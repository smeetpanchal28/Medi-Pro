...
/*
sid, student_first_name, student_last_name, student_email,
student_number, student_address, student_branch, student_dob,
student_college, student_gender, stream_id, created_at,
...
class Student {
    ...
    {
        $sql  = "SELECT * FROM student WHERE deleted = 0 ".$condition;
        $result_set = $this->connection->query($sql);
        return $result_set;
    }
    ...
    public function displayStudentsWithPagination($student_per_page){
        ...
        if(isset($_POST['key'])){
            $key = $_POST['key'];
            $condition = "AND (student_first_name like '%$key%' or
            student_last_name like '%$key%' or student_number like '%$key%') ";
        }
